...
- **The Tailwind binary is pinned and SHA256-verified.** `scripts/install-tailwind.sh` holds `TAILWIND_VERSION=v4.3.0` and a per-platform SHA256 table at the top. Bumping the binary is a deliberate edit to that script — copy the new release's `sha256sums.txt` values into the script, change `TAILWIND_VERSION`, delete `./bin/tailwindcss` if it exists locally, and run `just css-build` to verify. The script is invoked the same way from the host (`just css`/`just css-build`) and the prod `Dockerfile` (`css-builder` stage). The dev compose `tailwind` service reuses the same `css-builder` stage, so the binary is baked in at image build time — the running container calls `tailwindcss` directly from `/usr/local/bin` without re-invoking the install script.
