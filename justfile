...
up:
    GIT_SHA=$(git rev-parse --short HEAD 2>/dev/null || echo unknown) docker compose -f docker/compose.yaml -f docker/compose.dev.yaml up --build

up-prod:
    GIT_SHA=$(git rev-parse --short HEAD 2>/dev/null || echo unknown) docker compose -f docker/compose.yaml up --build

# Background mode; returns control to the shell instead of streaming logs.
up-d:
    GIT_SHA=$(git rev-parse --short HEAD 2>/dev/null || echo unknown) docker compose -f docker/compose.yaml -f docker/compose.dev.yaml up -d --build
